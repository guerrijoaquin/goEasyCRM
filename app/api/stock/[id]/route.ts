...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const allowed = ['name', 'sku', 'price', 'cost', 'stock', 'min_stock', 'category', 'channels'];
  ...
  const sql = `UPDATE products SET ${sets.map((k, i) => `${k}=$${i + 1}`).join(', ')}, updated_at=now()
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await query(`UPDATE products SET active=false WHERE id=$1 AND business_id=$2`, [id, payload.businessId]);
