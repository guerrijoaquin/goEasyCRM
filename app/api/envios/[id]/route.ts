...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const allowed = ['status', 'courier', 'tracking_number', 'eta', 'notes'];
  ...
  const sql = `UPDATE orders SET ${sets.map((k, i) => `${k}=$${i + 1}`).join(', ')}, updated_at=now()
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await query(`DELETE FROM orders WHERE id=$1 AND business_id=$2`, [id, payload.businessId]);
